...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        actionnode = set() # New Action Node
        for action in actions:
            a = PgNode_a(action)
            if a.prenodes.issubset(state):
                actionnode.add(a) 
                for s in state:
                    s.children.add(a)
                    a.parents.add(s)
        self.a_levels.append(actionnode)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for s in node_a1.action.effect_rem:
            if s in node_a2.action.effect_add:
                return True
        for s in node_a2.action.effect_rem:
            if s in node_a1.action.effect_add:
                return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for s in node_a1.action.effect_rem:
            if s in node_a2.action.precond_pos:
                return True
        for s in node_a1.action.effect_add:
            if s in node_a2.action.precond_neg:
                return True        

        for s in node_a2.action.effect_rem:
            if s in node_a1.action.precond_pos:
                return True
        for s in node_a2.action.effect_add:
            if s in node_a1.action.precond_neg:
                return True 
